...
방문 판매원 A는 X번 회사에 가서 물건을 판매하기 전에 먼저 소개팅 상대의 회사에 찾아 가서 함께 커피를 마실 예정이다.
...
이 때 방문 팡매원 A가 최종적으로 4번 회사에 가는 경로를 (1번 - 3번 - 5번 - 4번)으로 설정하면, 소개팅에도 참석할 수 있으면서 총 3만큼의 시간으로 이동할 수 있다.
...
문제풀이
플로이드 워셜 알고리즘 이용
1번 노드에서 X를 거쳐 K로 가는 최단거리(1번 노드에서 X까지의 최단 거리 + X에서 K까지의 최단 거리)
'''
